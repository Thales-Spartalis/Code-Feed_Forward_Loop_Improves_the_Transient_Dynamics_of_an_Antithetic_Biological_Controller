...
for niter = 1:N
    ...
    Vpar.beta_CB = par.beta_CB*Sample(niter,9);
    
    Reference(niter) = Vpar.mu/Vpar.alpha_B;
    
    options = odeset('RelTol',1e-10,'AbsTol',1e-10);
    x0 = [0 0 0];
    [t,x] = ode23s(@FeedbackCB,tspan,x0,options,Vpar);
    L = length(tspan);
    ...
    x02 = [n_a n_b n_c];
    [t2,x2] = ode23s(@FeedbackCB,tspan,x02,options,Vpar);
   ...
   %%% Max
    Max_p_FB(niter) = (max_Bact - Bact(end))/Bact(end); %/par.Px_tot;
...
%             rise = (t90(end) - t10(1))*t_increment; 
            ...
            if ~isempty(t_end1) && ~isempty(t_end2)
                S_time_FB(niter) = max(t_end1(end),t_end2(end))*delta_t(1); 
                ss = max(t_end1(end),t_end2(end));
            elseif isempty(t_end1) && isempty(t_end2) 
                S_time_FB(niter) = locmax_Bact*delta_t(1); 
                ss = locmax_Bact;
            elseif isempty(t_end1)
                S_time_FB(niter) = t_end2(end)*delta_t(1);
                ss = t_end2(end);
            elseif isempty(t_end2) 
                S_time_FB(niter) = t_end1(end)*delta_t(1);
                ss = t_end1(end);
            end
            ...
            if de_avg <= ss_tolerance %%% mins value got from biomolecules paper
                ...
                RelativeE_FB(niter) = (Bact(end) - Reference(niter))/Reference(niter);
...
save('FB_Global_SA_StepChange100.mat','Sample','RelativeE_FB',...
  'Max_p_FB','S_time_FB','success_trial', 'fail_trial','ss_diff','Gene_A','Gene_B','Gene_C','-v7.3')
